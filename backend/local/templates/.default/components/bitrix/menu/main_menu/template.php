						...
						?>
							...
							<?foreach($arRow as $itemIdLevel_2=>$arLevel_3):?>  <!-- second level-->
								...
								<?if (is_array($arLevel_3) && count($arLevel_3) > 0):?>
									...
									<?foreach($arLevel_3 as $itemIdLevel_3 => $arLevel_4):?>	<!-- third level-->
										<li class="bx-nav-3-lvl">
											...
											>
												<span class="bx-nav-3-lvl-link-text" style="color:blue"><?=$arResult["ALL_ITEMS"][$itemIdLevel_3]["TEXT"]?></span>
											</a>
                                            <?if (is_array($arLevel_4) && count($arLevel_4) > 0):?>
                                                <ul class="bx-nav-list-3-lvl">
                                                    <?foreach($arLevel_4 as $itemIdLevel_4):?>	<!-- 4nd level-->
                                                        <li class="bx-nav-3-lvl">
                                                            <a
                                                                    class="bx-nav-3-lvl-link"
                                                                    href="<?=$arResult["ALL_ITEMS"][$itemIdLevel_4]["LINK"]?>"
                                                                <?if ($existPictureDescColomn):?>
                                                                    onmouseover="window.obj_<?=$menuBlockId?> && obj_<?=$menuBlockId?>.changeSectionPicure(this, '<?=$itemIdLevel_4?>');return false;"
                                                                <?endif?>
                                                                    data-picture="<?=$arResult["ALL_ITEMS"][$itemIdLevel_4]["PARAMS"]["picture_src"]?>"
                                                                    <?if($arResult["ALL_ITEMS"][$itemIdLevel_4]["SELECTED"]):?>class="bx-active"<?endif?>
                                                            >
                                                                <span class="bx-nav-3-lvl-link-text"><?=$arResult["ALL_ITEMS"][$itemIdLevel_4]["TEXT"]?></span>
                                                            </a>
                                                        </li>
                                                    <?endforeach;?>
                                                </ul>
