;(function() {
	...
	BX.Sale.BasketComponent = {
		...
		bindSelected: function(){
			BX.bind(BX('total-selected'), 'click', BX.proxy(this.checkTotalSelected, this));
			BX.bind(BX('remove-all-items'), 'click', BX.proxy(this.removeAllSelected, this));
		},

		checkTotalSelected: function (){
			var checkedInput = BX.findChildren(BX(this.ids.basketRoot), {"tag": "input", 'class': "position-static"}, true);

			checkedInput.forEach(function(input){
				BX.adjust(input, { props: {checked: BX('total-selected').checked} });
			});
		},
		...
		removeAllSelected: function() {
			...
			checkedInput.forEach(function(selected){

				if(selected.checked){
					var itemData = this.items[selected.value];
		...
		bindBasketItemEvents: function(itemData)
		{
			...
			if (BX.type.isDomNode(itemNode))
			{
				...
				this.bindItemFavoriteEvents(itemNode, itemData);
			}
		},

		bindItemFavoriteEvents: function(node, data){
			if (!node || !data || !this.isItemAvailable(data.ID))
				return;
			var entity;

			var block = this.getEntity(node, 'basket-item-favorite-block');
			if(block) {
				entity = this.getEntity(block, 'basket-item-favorite-button');
				BX.bind(entity, 'click', BX.proxy(this.setFavorite, this));
			}
		},

		setFavorite: function ({target}){
			this.favoriteUrl = '/local/ajax/favorite.php?id=' + target.id + '&action=add';
			BX.ajax({
				method: 'GET',
				dataType: 'html',
				url: this.favoriteUrl,
				onsuccess: function (result){
					var favorite = BX('basket-item-favorite-span-'+target.id);
					...
					if(result.status == 'add'){
						...
						BX.removeClass(favorite, 'btn-fav__active');
					}
