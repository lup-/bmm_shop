...
foreach ($this->basketItems as $row)
{
	...
	if($row['PROPERTY_ISBN_VALUE']){
        $rowData['ISBN'] = ['CODE' =>  $row['PROPERTY_ISBN_VALUE']];
    }
    if($row['PROPERTY_AUTHOR_VALUE']){
        $rowData['AUTHOR'] = ['NAME' =>  $row['PROPERTY_AUTHOR_VALUE']];
    }

    if($row['AVAILABLE_QUANTITY']){
        for($quantity = 1; $quantity <= $row['AVAILABLE_QUANTITY']; $quantity++) {
            $rowData['SELECT_QUANTITY'][] = ['VALUE' => $quantity, "SELECTED" => $row['QUANTITY'] == $quantity ? 'selected' : ''];
        }
    }
		...
		foreach ($result['GRID']['HEADERS'] as &$value)
		{
			...
			if ($value['id'] === 'DETAIL_PICTURE')
			{
				...
				if (!empty($row['DETAIL_PICTURE_SRC']))
				{
                    ...
                    $rowData['DETAIL_PICTURE_SRC'] = $row['DETAIL_PICTURE_SRC'];
				}
			}
