...
<nav class="catalog-footer__pagination">
    ...
    <?else:?>
        <ul>
            <?if ($arResult["NavPageNomer"] > 1):
                if($arResult["bSavePage"]):?>
                    <li class="catalog-footer__pagination_prev page-item"><a class="page-link" href="<?=$arResult["sUrlPath"]?>?<?=$strNavQueryString?>PAGEN_<?=$arResult["NavNum"]?>=<?=($arResult["NavPageNomer"]-1)?>"><?=GetMessage("MAIN_UI_PAGINATION__PREV")?></a></li>
                <?else:
                    if ($arResult["NavPageNomer"] > 2):?>
                        <li class="catalog-footer__pagination_prev page-item"><a class="page-link" href="<?=$arResult["sUrlPath"]?>?<?=$strNavQueryString?>PAGEN_<?=$arResult["NavNum"]?>=<?=($arResult["NavPageNomer"]-1)?>"><?=GetMessage("MAIN_UI_PAGINATION__PREV")?></a></li>
                    <?else:?>
                        <li class="catalog-footer__pagination_prev page-item"><a class="page-link" href="<?=$arResult["sUrlPath"]?><?=$strNavQueryStringFull?>"><?=GetMessage("MAIN_UI_PAGINATION__PREV")?></a></li>
                    <?endif;
                endif;
            else:?>
            <li class="catalog-footer__pagination_prev page-item"><span class="page-link"><?=GetMessage("MAIN_UI_PAGINATION__PREV")?></span>
            <?endif;?>

                <? if ($arResult["NavPageNomer"] > 1):
				if ($arResult["nStartPage"] > 1):
					if($arResult["bSavePage"]):?>
                        <li class="page-item"><a class="page-link" href="<?=$arResult["sUrlPath"]?>?<?=$strNavQueryString?>PAGEN_<?=$arResult["NavNum"]?>=1"></a></li><?
					else:?>
			...
			do
			{
				...
				else:?><li class="page-item"><a class="page-link" href="<?=$arResult["sUrlPath"]?>?<?=$strNavQueryString?>PAGEN_<?=$arResult["NavNum"]?>=<?=$arResult["nStartPage"]?>"><?=$arResult["nStartPage"]?></a></li>
                <?endif;
				$arResult["nStartPage"]++;
			}
