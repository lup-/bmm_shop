...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		bindEvents: function()
		{
			...
			//убрала скрол для блока офрмления заказа, скользит))))
		...
		editFadeBasketBlock: function(activeNodeMode)
		{
			...
			else
			{
				...
				basketTable = BX.create('DIV', {props: {className: 'list-group list-group-flush basket-info__items bx-soa-item-table'}});
		...
		createBasketItem: function(basketItemsNode, item, index, active)
		{
			...
			basketItemsNode.appendChild(
				BX.create('DIV', {
					props: {className: 'list-group-item d-flex justify-content-between p-2 mb-3 bg-light'},
					children: cols
				})
			);
		...
		createBasketItemImg: function(data)
		{
			...
			return BX.create('DIV', {
				props: {className: 'basket-info__items_image mr-3'},
				children: [logoNode]
			});
		},
		...
		{
			var itemName = data.NAME || '', itemAuthor = data.PROPERTY_AUTHOR_VALUE || '',
				titleHtml = this.htmlspecialcharsEx(itemName),
				authorHtml = this.htmlspecialcharsEx(itemAuthor),
				props = data.PROPS || [],
				propsNodes = [];
			...
			/*return BX.create('DIV', {
				props: {className: 'd-flex flex-column justify-content-between flex-fill'},
				children: propsNodes.length ? [
					...
					BX.create('DIV', {props: {className: 'basket-info__items_title'}, html: titleHtml})
				]
			});*/
			return BX.create('DIV', {
				props: {className: 'd-flex flex-column justify-content-between flex-fill'},
				children: [
					BX.create('DIV', {props: {className: 'basket-info__items_title'}, html: titleHtml}),
					BX.create('DIV', {props: {className: 'basket-info__items_author'}, html: authorHtml}),
					BX.create('DIV', {props: {className: 'flex-fill'}}),
					BX.create('DIV', {props: {className: 'd-block d-sm-none'},html: '1&nbsp;шт.'})
				]
			});
		},
		...
		{
			var totalValue, className = 'list-group-item d-flex justify-content-between align-items-center';
