...
$resProducts =  \Bitrix\Iblock\ElementTable::getList([
    ...
    'filter' => [
        "IBLOCK_ID" => [$_ENV['BOOK_BLOCK_ID'], $_ENV['NON_BOOK_BLOCK_ID'], $_ENV['FOOD_BLOCK_ID']],
        'PRICE_LIST.CATALOG_GROUP_ID' => 1
    ],
])->fetchAll();
...
for( $i = 0; $i <= count($offers)/100; $i++ ){
    ...
    foreach($stocks as $stock){
        if($offers[$stock->idtow]){
            $elementID = $offers[$stock->idtow]->id;
            if($stock->price <> $offers[$stock->idtow]->price){
                ...
                $obPrice = new \Bitrix\Catalog\Model\Price;
                if ($arPrice = $dbPrice->Fetch()) {
                    $result = $obPrice->Update($arPrice["ID"], $priceFields);
                } else {
                    $obPrice->Add($priceFields);
                }
            }
