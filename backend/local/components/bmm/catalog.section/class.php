...
class CatalogSectionComponent extends ElementList
{
	...
	public function onPrepareComponentParams($params)
	{
        ...
        $params = $this->prepareElementSortRow(
            $params,
            ['ORDER' => 'ELEMENT_SORT_FIELD3', 'DIRECTION' => 'ELEMENT_SORT_ORDER3'],
    ...
    protected function getSort()
    {
        ...
        $field = mb_strtoupper($this->arParams['ELEMENT_SORT_FIELD3']);
        if (!isset($sortFields[$field]))
        {
            $sortFields = array_merge(
                array_slice( $sortFields, 0, 2, true ),
                array( $field => $this->arParams['ELEMENT_SORT_ORDER3'] ),
                array_slice( $sortFields, 2, null, true )
            );
